...
despesas.educacao    = parseFloat(prompt("Valor da Educação (Mensalidades/Cursos): R$ "));

console.log("\n--- Despesas Registradas ---");

// Exibe cada despesa, validando valores
for (const categoria in despesas) {
  if (!isNaN(despesas[categoria]) && despesas[categoria] >= 0) {
    ...
    despesas[categoria] = 0; // Define como 0 para não atrapalhar o cálculo
  }
}
...
// --- Cálculo da Média ---
const numeroDeCategorias = Object.keys(despesas).length;
const mediaDespesas = totalDespesas / numeroDeCategorias;
console.log(`Média de Despesas: R$ ${mediaDespesas.toFixed(2)}`);